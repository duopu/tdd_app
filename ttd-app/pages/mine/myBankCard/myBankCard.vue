	...
	<view class="my-bank-card">
		...
		<back-container>
          ...
          <uni-icons v-else type="circle" size="22" color="#969799" />
				...
				<bank-card-item v-for="(item, i) in cardList" :key="i"
					:i="isSelect ? (selectCard == item.bankCardNo ? 1 : 3) : 2 " :item="item" @click="editCard(item)" />
			</view>
		</back-container>
		...
		methods: {
			...
			editCard(card) {
				if (this.isSelect) {
					...
					uni.navigateTo({
						url: `/pages/mine/bankAccount/bankAccount?id=${card.id}`
					})
				}
			},
			addCard() {
