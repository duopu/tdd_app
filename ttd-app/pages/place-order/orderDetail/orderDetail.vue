	...
	<view>
		...
		<iphonex-bottom>
			...
			<view v-if="isPlaceOrder" class="order-dtl-btn-box">
				<view class="order-dtl-btn1" v-if="[10, 20, 30].includes(order.state)" @click="cancelOrderTip">取消订单
				</view>
				<!-- 待报价 待确认 -->
				<view class="order-dtl-btn1"
					v-if="[10, 20].includes(order.state) || (order.state == 30 && order.subState == 4)"
					@click="toQuestionPage">查看问题</view>
				<!-- 待确认 -->
				<view class="order-dtl-btn1 order-dtl-btn2" v-if="order.state == 20 && order.subState == 3"
					@click="toChoosePrice">选价</view>
				...
				<view class="order-dtl-btn1 order-dtl-btn2" v-if="order.state == 30 && order.subState == 5"
					@click="toOrderWork">确认开始</view>
				<!-- 待完工 -->
				<view class="order-dtl-btn1 order-dtl-btn2" v-if="order.state == 40 && order.subState == 6"
					@click="toPayPartMoney">部分付款</view>
				<view class="order-dtl-btn1 order-dtl-btn2" v-if="order.state == 40 && order.subState == 7"
					@click="toOrderWork">确认完工</view>
				<!-- 已完工 -->
			...
			<view v-else class="order-dtl-btn-box">
				...
				<!-- 待开始 -->
				<view class="order-dtl-btn1"
					v-if="[30, 40].includes(order.state) && order.subState != 7 && order.receiverType == 2"
					@click="toReviewTeam">变更人员</view>
				<view class="order-dtl-btn1" v-if="[30, 40, 50].includes(order.state)" @click="toComplainPage">投诉</view>
				<view class="order-dtl-btn1 order-dtl-btn2" v-if="[30].includes(order.state) && order.subState == 4"
...
<script>
	...
	export default {
		...
		methods: {
			...
			queryCommentList() {
				this.$http
					.post('/b/ordercomment/queryByReceiverOrderId', {
						id: this.id
					})
					.then(res => {
