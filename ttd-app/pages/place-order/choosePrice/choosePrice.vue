			...
			<view class="chp">
				<view class="chp-1" v-for="(item, index) in quoteList" :key="index" @click="toQuoteDetail(item)">
					<view class="chp-11">
						...
						<image v-if="showSelect(item)" @tap.stop="onChange(item)"
							src="https://ttd-public.obs.cn-east-3.myhuaweicloud.com/app-img/mine/checkBoxChecked.svg"
							class="chp-193-right3" />
						<image v-else @tap.stop="onChange(item)"
							src="https://ttd-public.obs.cn-east-3.myhuaweicloud.com/app-img/mine/checkBoxEmpty.svg"
							class="chp-193-right3" />
		...
		<iphonex-bottom>
			<view class="chp-b1">
				<view class="chp-b2">
					...
					<text class="chp-b21">份（已报 {{ showCount(1) }} 项，未报 {{ showCount(2) }} 项，重复 {{ showCount(3) }}
						项）</text>
				</view>
				<view class="chp-b3">
			...
			<template #slot1>
				<view class="choose-price-slot">
					...
					<view class="checked-bsx">
						<checkd-item :label="' '" :value="remainItemSetting"
							@change="((value) => { remainItemSetting = value })"
							:list="[{ text: '未报项单独发包', value: 2 }, { text: '关闭未报项', value: 1 }]" />
					</view>
				</view>
			</template>
		</modal-box>
...
<script>
	...
	export default {
		...
		methods: {
			queryQuoteList(id) {
				this.$http.post('/b/orderquote/choosePriceList', {
						id: this.id
					}, true)
					.then(res => {
						this.quoteList = res;
