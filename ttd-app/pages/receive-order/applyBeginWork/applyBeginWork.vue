	...
	<view>
		...
		<view class="abw-progress">
			...
			<view class="abwp-body">
				<view class="abwp-l">
					<image class="abwp-lt1"
						src="https://ttd-public.obs.cn-east-3.myhuaweicloud.com/app-img/mine/applyJob1.svg" />
					<view class="abwp-lt3" />
					<image v-if="order.subState == 6 || order.subState == 7" class="abwp-lt1"
						src="https://ttd-public.obs.cn-east-3.myhuaweicloud.com/app-img/mine/applyJob.svg" />
				</view>
				<view class="abwp-r">
					<view class="abwp-r1">
						<text class="abwp-r1l">开始工作</text>
						<text v-if="startApplyInfo.address.latitude && order.subState == 4" class="abwp-r1r"
							@click="checkLocationPermission(1)">重新定位</text>
					</view>

					<map v-if="startApplyInfo.address.latitude" class="abwp-rmap"
						:latitude="startApplyInfo.address.latitude" :longitude="startApplyInfo.address.longitude" />
					<view v-else class="abwp-bo-1" @click="checkLocationPermission(1)">
						<image src="https://ttd-public.obs.cn-east-3.myhuaweicloud.com/app-img/mine/iconCircleAdd.svg"
							class="apply-add-jia" />
						<view class="abwp-bo-1text">添加位置</view>
					</view>

					<view class="abwp-rmap-text">
						{{ startApplyInfo.address.province || '' }} {{ startApplyInfo.address.city || '' }}
						{{ startApplyInfo.address.district || '' }} {{ startApplyInfo.address.address || '' }}
					</view>

					<view v-if="isPlaceOrder" class="abwp-rtime">{{ startApplyInfo.applyTime }}</view>

					<!-- 上传文件 -->
					<up-file v-model="startApplyInfo.resourceList" :modal="order.subState != 4 ? 'show' : 'select'"
						:showAudio="false" />
					...
					<text v-if="completeApplyInfo.address.latitude && order.subState == 6" class="abwp-rend-text"
						@click="checkLocationPermission(2)">重新定位</text>
				</view>
			</view>

			<view v-if="order.subState == 6 || order.subState == 7" class="abwp-bottom">
				<map v-if="completeApplyInfo.address.latitude" class="abwp-bo-1"
					:latitude="completeApplyInfo.address.latitude" :longitude="completeApplyInfo.address.longitude" />
				...
				<view class="abwp-rmap-text">
					{{ completeApplyInfo.address.province || '' }} {{ completeApplyInfo.address.city || '' }}
					{{ completeApplyInfo.address.district || '' }} {{ completeApplyInfo.address.address || '' }}
				</view>

				<view v-if="isPlaceOrder" class="abwp-rtime">{{ completeApplyInfo.applyTime }}</view>

				<!-- 上传文件 -->
				<up-file v-model="completeApplyInfo.resourceList" :modal="order.subState != 6 ? 'show' : 'select'"
					:showAudio="false" />
			</view>
		</view>
		...
		<iphonex-bottom>
			<big-btn v-if="isPlaceOrder" :button-text="order.subState == 5 ? '确认开始' : '确认完工'"
				@click="approveBeginWork" />
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.title = option.type == 'start' ? '申请开始工作' : '申请结束工作'
			if (option.id) {
				this.id = option.id;
				this.queryOrderInfo();
				this.queryApplyInfo();
		...
		methods: {
			...
			queryApplyInfo() {
				this.$http.post('/b/orderreceive/queryApplyDetail', {
						id: this.id
					}, true)
					.then(res => {
			...
			applyBeginWork() {
				...
				const params = {
					...
					resourceList: this.order.subState == 4 ? this.startApplyInfo.resourceList : this.completeApplyInfo
						.resourceList,
					orderAddress: this.order.subState == 4 ? this.startApplyInfo.address : this.completeApplyInfo
						.address,
				};
