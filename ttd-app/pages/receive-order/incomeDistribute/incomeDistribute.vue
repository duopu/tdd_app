...
<script>
	...
	export default {
		...
		methods: {
			...
			queryTotalAmount() {
				this.$http.post('/b/ordersettlement/queryCanSettlementAmount', {
						id: this.id
					}, true)
					.then(res => {
			...
			queryAlreadyDistribute() {
				this.$http.post('/b/ordersettlement/queryTeamMemberDistributorAmountList', {
						id: this.id
					}, true)
					.then(res => {
						this.memberList = res;
			...
			// 队员确认分配收益
			confirmDistribute() {
				this.$http.post('/b/ordersettlement/confirm', {
						id: this.id
					}, true)
					.then(res => {
						uni.showToast({
			...
			// 队员拒绝分配收益
			refuseDistribute() {
				this.$http.post('/b/ordersettlement/refuse', {
						id: this.id
					}, true)
					.then(res => {
						uni.showToast({
