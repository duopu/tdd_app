<template>
	<view>
			...
			<view class="qus-detail">
				<view class="qus-it" v-for="(question, index) in questionList" :key="index">
					...
					<view class="qus-1 qus-2">
						<image src="https://ttd-public.obs.cn-east-3.myhuaweicloud.com/app-img/mine/reply.svg"
							class="qus-1image" />
						<view class="qus-1right">
							<view class="qus-1right-top">
							...
							<view v-if="question.answerList.length > 0" class="qus-2text">
								{{ question.answerList[0].content || '' }}</view>
							<view class="qus-1img-list">
								<template
									v-if="question.answerList.length > 0 && question.answerList[0].pictureList.length > 0">
									<image :src="p" class="qus-1img-item"
										v-for="p in question.answerList[0].pictureList" :key="p" />
								</template>
		...
		<ask-questions-model ref="askQuestionsModel" :showType="!isPlaceOrder" 
...
<script>
	...
	export default {
		...
		methods: {
			queryQuestionList() {
				this.$http.post('/b/orderquestionanswer/queryList', {
