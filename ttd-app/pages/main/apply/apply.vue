...
<template>
	<view class="page-container apply">
		...
		<scroll-view scroll-y="true" class="scroll-content">
			...
			<view class="white"><textarea class="profile-text" maxlength="200" placeholder="请输入简介" placeholder-class="input-placeholder" v-model="remark" /></view>
			<!-- 从业信息 -->
			...
			<view class="white list m-top-16">
				...
				<view class="list-item flex-center-between" v-for="(item, index) in skillData" :key="index">
					...
					<image src="../../../static/delete.png" mode="aspectFill" class="image-delete" @click="deleteSkill(index)"></image>
				</view>
			...
			<view class="white list m-top-16">
				...
				<view class="list-item flex-center-between" v-for="(item, index) in userroleData" :key="index">
					...
					<image src="../../../static/delete.png" mode="aspectFill" class="image-delete" @click="deleteSoftwareconf(index)"></image>
				</view>
			...
			<view class="white list m-top-16">
				...
				<view class="list-item flex-center-between" v-for="(item, index) in projectData" :key="index">
					...
					<image src="../../../static/delete.png" mode="aspectFill" class="image-delete" @click="deleteProject(index)"></image>
				</view>
			...
			<view class="white list m-top-16">
				...
				<view class="list-item flex-center-between" v-for="(item, index) in toolData" :key="index">
					...
					<image src="../../../static/delete.png" mode="aspectFill" class="image-delete" @click="deleteTool(index)"></image>
				</view>
...
export default {
	components: { AddTechnologyPopup, AddStaffPopup, AddToolsPopup, AddProjectPopup },
	data() {
		return {
			name: '',
			phone: '',
	...
	methods: {
		...
		submitApply() {
			...
			if(!this.name){
				...
				param.skillApplyList = this.skillData.map(s=>({skillId:s.id,nodeLink:s.name,brandList:s.brandList}));
				param.userRoleApplyList = this.userroleData.map(u=> ({userRoleId:u.id,nodeLink:u.name,softwareList:u.softwareconfList}));
				param.projectApplyList = this.projectData.map(p=>({projectName:p.name,resume:p.content}));
				param.toolApplyList = this.toolData.map(t => ({toolId:t.id,nodeLink:t.name,leaseFlag:t.leaseFlag}));
