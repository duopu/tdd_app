...
<template>
	<view class="page-container flex-column-center">
		...
		<view class="name-view flex">
			...
			<input type="text" v-model="phone" placeholder="注册请输入姓名" />
		</view>
		<view class="name-view flex">
		...
		<button class="btn primary" @click="loginAction">登录</button>
	...
	export default {
		...
		methods: {
			...
			loginTodo() {
				...
				this.$http.post('/oauth/phoneLogin4Mini',param,true).then(res=>{
					const user = {
						...res.user,
						token: res.token,
						openId: res.openid,
